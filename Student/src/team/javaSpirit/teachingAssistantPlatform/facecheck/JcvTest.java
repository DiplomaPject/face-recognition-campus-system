...
public class JcvTest {
	...
	public static String captureFaceBySid(List<String> sids) throws Exception, InterruptedException, IOException {
		/**
		 * <p>
		 * Title:initTrain
		 * </p>
		...
		while (true) {
			...
			if (c) {
				c=false;
				imagesrc = recordCamera.recordCameraBySid(sids, grabber,canvas);
				canvas.dispose();
			}
			if (!canvas.isDisplayable()) {
				grabber.stop();// 停止抓取
